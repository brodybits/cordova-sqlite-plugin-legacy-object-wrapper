...
#### SQLite plugin db connection handle object is defined by a constructor function and prototype member functions:
    ...
    SQLitePlugin::open = (success, error) ->
      console.log 'OPEN database with name: ' + @dbname
      if @dbname of @openDBs
        ...
        # XXX TEMPORARY & DEPRECATED location value:
        console.log 'resolve path now'
        # resolvePathOptions = { name: @dbname, location: 2 }
        name = @dbname
        window.sqliteStorageFile.resolveAbsolutePath { name: name, location: 2 }, (path) =>
          console.log 'resolve path cb received'
          console.log 'path: ' + path
          opts = { fullName: path, flags: 6 }
          window.sqliteBatchConnectionManager.openDatabaseConnection opts, (id) =>
...
## SQLite plugin transaction object for batching:
    ...
    SQLitePluginTransaction::run = ->
      ...
      window.sqliteBatchConnectionManager.executeBatch id, tropts, (batchResults) =>
